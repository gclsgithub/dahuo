...
public class FwBaseOut implements Serializable {
    ...
    final public static FwBaseOut ng(ResultMessages myresult) {
        FwBaseOut out = new FwBaseOut(ResultCd.NG);
        out.errors.addAll(Arrays.asList(myresult));
        return out;
    }
    ...
    final public static <T extends FwBaseOut> T ng(Class<T> type, ResultMessages resultMessage) {
        T out = ng(type);
        out.errors.add(resultMessage);
        return out;
    }
